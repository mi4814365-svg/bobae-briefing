...
```bash
pip install Pillow
apt-get install fonts-nanum      # 한글 서체 (맥·윈도우는 이미 있다)

python3 insta/caption.py 원본.jpg "탭샵바에서 헤어지자 하는 거니" \
    -o insta/media/2026-08-12.jpg --square --position band
```

| 옵션 | 설명 |
| --- | --- |
| `--position band` | 사진 **아래 검은 띠**에 글자를 넣는다. 얼굴을 안 가려서 인물 사진에 좋다 |
| `--position bottom` | 사진 위에 바로 얹는다 (기본). 검은 테두리 + 아래쪽 그늘로 읽히게 한다 |
| `--position top` | 위쪽에 얹는다 |
| `--square` | 1:1 로 가운데 자른다 |
| `--scale` | 글자 크기 / 가로폭 비율 (기본 `0.082`) |
| `--scrim` | 얹기 방식에서 배경을 어둡게 하는 정도 0~255 (기본 150) |

알아서 해주는 것 두 가지:

- **휴대폰 사진의 EXIF 방향을 픽셀에 반영한다.** 이걸 안 하면 세로로 찍은
  사진에 글자를 얹었을 때 누운 그림이 나온다.
- **줄을 고르게 끊고, 띠가 4:5 를 넘지 않게 글자를 줄인다.** `거니` 한 마디만
  마지막 줄에 떨어지거나, 인스타가 위아래를 잘라먹는 일이 없다.
