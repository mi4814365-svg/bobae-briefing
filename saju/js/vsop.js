// VSOP87D 지구 황경(L)·동경거리(R) 절단 급수 — Meeus, Astronomical Algorithms 부록 III.
// 계수 단위: L은 1e-8 rad, R은 1e-8 AU. t는 J2000 기준 율리우스 천년(TT).
const L0 = [
...
const series = (terms, t) => terms.reduce((s, [a, b, c]) => s + a * Math.cos(b + c * t), 0);

/** 지구 일심 황경(rad)과 거리(AU). jde = TT 율리우스일 */
export function earthHeliocentric(jde) {
  const t = (jde - 2451545) / 365250;
  const L = (series(L0, t) + series(L1, t) * t + series(L2, t) * t ** 2 + series(L3, t) * t ** 3
    + series(L4, t) * t ** 4 + series(L5, t) * t ** 5) / 1e8;
  const R = (series(R0, t) + series(R1, t) * t + series(R2, t) * t ** 2) / 1e8;
  return { L, R };
}
...
/** 태양 시황경(도), FK5·장동·광행차 보정 포함. Meeus ch.25 고정밀 */
export function sunApparentLongitudeVSOP(jde) {
  const { L, R } = earthHeliocentric(jde);
  const T = (jde - 2451545) / 36525;
  let theta = norm360(L / RAD + 180);
  // FK5 보정
  const lambdaP = theta - 1.397 * T - 0.00031 * T * T;
  theta += -0.09033 / 3600;
  void lambdaP;
  // 장동 Δψ (Meeus ch.22 약식)
